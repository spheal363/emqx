...
-module(emqx_load_redirect_SUITE).
...
t_find_max_throughput_publisher(_Config) ->
    ...
    %% テスト実行
    {ok, ClientId2, 200} = emqx_load_redirect:find_max_throughput_publisher(),
...
t_find_low_cpu_nodes(_Config) ->
    ...
    %% テスト実行
    LowCpuNodes = emqx_load_redirect:find_low_cpu_nodes(),
...
t_format_server_references(_Config) ->
    ...
    ServerRefs = emqx_load_redirect:format_server_references(Nodes),
...
t_node_to_address(_Config) ->
    ...
    Addr1 = emqx_load_redirect:node_to_address(Node1),
    Addr2 = emqx_load_redirect:node_to_address(Node2),
...
t_disconnect_publisher(_Config) ->
    ...
    %% テスト実行
    ok = emqx_load_redirect:disconnect_publisher(ClientId, ServerRefs),
...
t_should_redirect(_Config) ->
    %% 初回実行時はtrue
    State1 = #emqx_load_redirect_state{last_redirect_time = undefined},
    ?assert(emqx_load_redirect:should_redirect(State1)),

    %% 30秒以内はfalse
    State2 = #emqx_load_redirect_state{last_redirect_time = erlang:system_time(millisecond)},
...
t_maybe_redirect_publisher(_Config) ->
    %% 正常なリダイレクト処理のテスト
    ok = emqx_load_redirect:maybe_redirect_publisher(),
